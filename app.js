...
const Profile = require("./src/profiledb");
...
app.post("/signup", async (req, res) => {
  ...
  if (!userExists) {
    ...
    res.send(
      "<script>alert('user already exits');window.location.href = '/'</script>"
    );
  }
});
...
app.post("/login", async (req, res) => {
  try {
    ...
    res.send(
      "<script>alert('Wrong details');window.location.href = '/'</script>"
    );
  }
});
...
app.get("/update/:custom", (req, res) => {
  if (req.session.username) {
    PosT.findById(req.params.custom, (err, result) => {
      console.log(result);
      if (
        req.session.username === result.author ||
